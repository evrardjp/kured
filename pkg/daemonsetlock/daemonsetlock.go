...
type Lock interface {
	Acquire(interface{}) (bool, string, error)
	Release() error
	Holding() (bool, error)
}
...
func (dsl *DaemonSetSingleLock) Acquire(metadata interface{}) (bool, string, error) {
	for {
		...
		value := lockAnnotationValue{NodeID: dsl.nodeID, Metadata: metadata, Created: time.Now().UTC(), TTL: dsl.TTL}
...
func (dsl *DaemonSetSingleLock) Holding() (bool, error) {
	ds, err := dsl.GetDaemonSet(k8sAPICallRetrySleep, k8sAPICallRetryTimeout)
	if err != nil {
		return false, fmt.Errorf("timed out trying to get daemonset %s in namespace %s: %w", dsl.name, dsl.namespace, err)
	}
...
func (dsl *DaemonSetMultiLock) Acquire(metadata interface{}) (bool, string, error) {
	for {
		ds, err := dsl.GetDaemonSet(k8sAPICallRetrySleep, k8sAPICallRetryTimeout)
		if err != nil {
			return false, "", fmt.Errorf("timed out trying to get daemonset %s in namespace %s: %w", dsl.name, dsl.namespace, err)
		...
		lockPossible, newAnnotation := dsl.canAcquireMultiple(annotation, metadata, dsl.TTL, dsl.maxOwners)
		if !lockPossible {
...
func (dsl *DaemonSetMultiLock) Holding() (bool, error) {
	ds, err := dsl.GetDaemonSet(k8sAPICallRetrySleep, k8sAPICallRetryTimeout)
	if err != nil {
		return false, fmt.Errorf("timed out trying to get daemonset %s in namespace %s: %w", dsl.name, dsl.namespace, err)
	}
